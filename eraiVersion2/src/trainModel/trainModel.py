...
from utilities.fileFolderManipulations import getParentFolder,createFolder
...
def trainLSTMModel(trainingDataFilePath, TIME_STEPS_CONFIG=60, BATCH_SIZE_CONFIG = 512, LEARNING_RATE_CONFIG=0.001, EPOCHS_CONFIG = 20, numberOfOutputs=4) :
    ...
    try:
        ...
        try:
            trainingData=trainingData.drop(columns={'Unnamed: 0'})
        except KeyError:
            pass
        ...
        x_t, y_t = build_timeseries(x_train, 3,numberOfOutputs,TIME_STEPS)
        x_t = trim_dataset(x_t, BATCH_SIZE)
        y_t = trim_dataset(y_t, BATCH_SIZE)
        x_temp, y_temp = build_timeseries(x_test, 3, numberOfOutputs,TIME_STEPS)
...
def build_timeseries(mat, y_col_index, numberOfOutputs, TIME_STEPS):  
    ...
    y = np.zeros((dim_0,numberOfOutputs))
    
    for i in range(dim_0):
        x[i] = mat[i:TIME_STEPS+i]
        # fetch first 'numberOfOutputs' column values of current row as output
        y[i] = mat[TIME_STEPS+i][0:numberOfOutputs] #[TIME_STEPS+i, y_col_index]
